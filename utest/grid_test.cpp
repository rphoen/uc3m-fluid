...
TEST(GridConstructorTest, ConstructorWithCorrectValues) {
  ...
  ASSERT_EQ(grid.get_numberX(), (Constants::boxUpperBound[0] - Constants::boxLowerBound[0]) / (Constants::radiusMultiplier / 10.0));
  ASSERT_EQ(grid.get_numberY(), (Constants::boxUpperBound[1] - Constants::boxLowerBound[1]) / (Constants::radiusMultiplier / 10.0));
  ASSERT_EQ(grid.get_numberZ(), (Constants::boxUpperBound[2] - Constants::boxLowerBound[2]) / (Constants::radiusMultiplier / 10.0));
  ASSERT_EQ(grid.get_numBlocks(), grid.get_numberX() * grid.get_numberY() * grid.get_numberZ());
  ASSERT_EQ(grid.get_sizeX(), (Constants::boxUpperBound[0] - Constants::boxLowerBound[0]) / grid.get_numberX());
  ASSERT_EQ(grid.get_sizeY(), (Constants::boxUpperBound[1] - Constants::boxLowerBound[1]) / grid.get_numberY());
  ASSERT_EQ(grid.get_sizeZ(), (Constants::boxUpperBound[2] - Constants::boxLowerBound[2]) / grid.get_numberZ());
...
// TEST(GridFindAdjacentBlocksTest, FindAdjacentBlocks) {
//   // Create a grid with 10 particles per million (ppm) and 1000 particles
//   Grid grid(10.0, 1000);

//   // Create a particle and find its block
//   Particle particle(4, {0.1, 0.2, 0.3}, {0.1, 0.2, 0.3}, {0.2, 0.4, 0.6});
//   Block centerBlock = grid.findBlock(particle);

//   // Find the adjacent blocks to the center block
//   grid.findAdjBlocks(centerBlock);

//   // Check that the center block has eight adjacent blocks
//   ASSERT_EQ(centerBlock.get_adjacentBlocks().size(), 8);
// }
